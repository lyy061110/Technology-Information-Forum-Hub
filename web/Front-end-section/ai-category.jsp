...
<script>
    ...
    function checkLoginStatus() {
        // 尝试从sessionStorage获取（前端存储）
        const localUsername = localStorage.getItem('username');
        const localLoggedIn = localStorage.getItem('isLoggedIn');

        // 尝试从服务器Session获取（如果页面是JSP，可以通过JSP代码获取）
        <%
            // JSP代码检查服务器Session
            ...
            if (userSession != null) {
                ...
                if (loggedInAttr != null && (Boolean)loggedInAttr) {
                    serverLoggedIn = true;
                    serverUsername = (String) userSession.getAttribute("username");
                }
            }
        %>

        // 合并检查结果
        let username = '<%= serverUsername != null ? serverUsername : "" %>';
        let isLoggedIn = <%= serverLoggedIn %>;

        // 如果服务器Session有效，优先使用
        if (username && username.trim() !== "") {
            console.log("使用服务器Session登录状态");
        ...
        // 否则使用localStorage
        else if (localLoggedIn === 'true' && localUsername) {
            console.log("使用localStorage登录状态");
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.title = `AI Forum - Welcome ${username}`;
    });
    ...
    function viewPost(postId) {
        console.log('查看帖子 ID:', postId);
        // 跳转到帖子详情页面
        window.location.href = 'post-detail.jsp?id=' + postId;
    }

    // 排序帖子
    function sortPosts(sortBy) {
        console.log('按排序方式:', sortBy);
        // 这里可以添加排序逻辑
        // 例如重新加载页面或Ajax请求
        alert('将按 ' + sortBy + ' 排序帖子');
    }
    ...
    function filterBySubcategory(subcategory) {
        console.log('筛选子分类:', subcategory);
        alert('将显示 ' + subcategory + ' 子分类的帖子');
    }

    // 查看热门话题
    function viewTrending(topic) {
        console.log('查看热门话题:', topic);
        // 这里可以跳转到相关话题页面
        alert('查看热门话题: ' + topic);
    }
</script>
